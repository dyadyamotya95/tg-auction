...
**Почему Svelte?** Конкурсная работа без требований к фронтенду. Svelte 5 с runes – современный подход к реактивности.

**Почему pnpm?** Монорепа из 10 пакетов. Все зависимости через `workspace:*` – изменения в `@tac/core` мгновенно видны в `@tac/api`.
...
apps/                          packages/
  ...
  │                              │
  ├─ bots/                       ├─ env/
  │   Demo боты (5 шт)           │   Конфиг + переменные окружения
  │   Перебивают ставки          │
  │   deps: @tac/db, env         └─ telegram/
  │                                  Валидация initData +
  └─ web/                            Push-уведомления (grammy)
      Svelte 5 UI
      deps: @tac/shared
```
...
1. **`@tac/core` – без внешних зависимостей**  
   Можно использовать с Fastify, Express, Deno. Вся бизнес-логика в одном месте.
...
Для демонстрации работы аукциона в проекте есть 5 ботов (Bot #1 ... Bot #5).

Поведение:
- Боты автоматически перебивают первую ставку реального пользователя
- Каждый бот ставит с задержкой 100-300ms после предыдущего
- Боты срабатывают только **один раз** за аукцион
...
**Что доступно в UI:**
- Пополнение баланса – нажатие на баланс добавляет +1000 ⭐
- История транзакций – пополнения, списания, возвраты
- Создание аукциона с настройкой раундов
- Участие в аукционе и перебивание ставок
- Профиль с выигранными подарками
...
# Bots (demo боты для перебивания ставок, optional) 
pnpm --filter @tac/bots dev
...
Vite проксирует `/api` на `localhost:3000`, поэтому достаточно одного ngrok на Web.
